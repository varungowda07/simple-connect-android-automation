...
import tests.login.*;
import tests.password.ForgotPasscodeScreen;
...
import tests.timefence.TimeFenceFlow;
...
public class MasterFlowTests extends BaseTest {
    ...
    ForgotPasscodeScreen forgotPasscodeScreen = new ForgotPasscodeScreen();
...
public void flow8_EditEmergencyConatct() {
    ...
    @Test(priority = 11)
    public void flow10_PasswordForgot() {
        try {
    ...
    @Test(priority = 12)
    public void flow11_SetTimeFence() {
        try {
         timeFenceFlow.timeFenceFlow();
        } finally {
            ensureLogout();
            sleep(5000);
            softAssert.assertAll();
